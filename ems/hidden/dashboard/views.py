...
def dashboard(request):
	...
	staff_leaves = Leave.objects.filter(name = user)
...
def dashboard_employees_create(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			print(user)
			#all = Employees.objects.filter(id = user)
			assigned_user = Employees.objects.get(id = user)

			assigned_user = all[0].name

			instance.user = assigned_user
			# under process 
			#print(assigned_user)
			#print(assigned_user.name)
...
def employee_edit_data(request,id):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			user = request.POST.get('user')
			assigned_user = Employee.objects.get(id = user)
...
def leaves_view(request,id):
	...
	#print(leave.name)
	employee = Employees.objects.filter(name = leave.name)[0]
	print(employee)
	return render(request,'dashboard/leave_detail_view.html',{'leave':leave,'employee':employee,'title':'{0}-{1} leave'.format(leave.name,leave.status)})
